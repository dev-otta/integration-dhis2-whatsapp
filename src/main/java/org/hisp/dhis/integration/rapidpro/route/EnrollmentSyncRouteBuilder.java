...
public class EnrollmentSyncRouteBuilder extends AbstractRouteBuilder {
    ...
    protected void doConfigure() throws Exception {
        from("servlet:tasks/enrollmentSync?muteException=true")
                ...
                .setBody(constant(
                        Map.of("status", "success", "data", "Synchronised RapidPro contacts with DHIS2 Enrollments")))
                .marshal().json();
        ...
        from("direct:enrollmentSync")
                ...
                .log(LoggingLevel.INFO, LOGGER,
                        "Synchronising {{dhis2.program.name}} enrollments with RapidPro contacts...")
                .to("direct:prepareTrackerRapidPro")
                .setHeader("CamelDhis2.queryParams",
                        constant(Map.of("program", "JA1vLYT8htI", "orgUnit", "DFn0SHhghQp", "skipPaging", "true")))
                ...
                .setProperty("nextContactsPageUrl",
                        simple("{{rapidpro.api.url}}/contacts.json?group={{dhis2.program.name}}"))
                ...
                .end()
                .log(LoggingLevel.INFO, LOGGER,
                        "Completed synchronisation of RapidPro contacts with {{dhis2.program.name}} enrollments");

        from("direct:createEnrollmentContact")
                .transform(datasonnet("resource:classpath:enrollmentContact.ds", Map.class, "application/x-java-object",
                        "application/x-java-object"))
                .setProperty("dhis2EnrollmentId", simple("${body['fields']['dhis2_enrollment_id']}"))
                .marshal().json().convertBodyTo(String.class)
                .setHeader("Authorization", constant("Token {{rapidpro.api.token}}"))
                .log(LoggingLevel.DEBUG, LOGGER,
                        "Creating RapidPro contact for {{dhis2.program.name}} enrollment ${exchangeProperty.dhis2EnrollmentId}")
                ...
                .log(LoggingLevel.WARN, LOGGER,
                        "Unexpected status code when creating RapidPro contact for DHIS2 enrollment ${exchangeProperty.dhis2EnrollmentId} => HTTP ${header.CamelHttpResponseCode}. HTTP response body => ${body}")
                .end();
        ...
        from("direct:updateEnrollmentContact")
                ...
                .log(LoggingLevel.WARN, LOGGER,
                        "Unexpected status code when updating RapidPro contact ${exchangeProperty.rapidProUuid} => HTTP ${header.CamelHttpResponseCode}. HTTP response body => ${body}")
                .end();
